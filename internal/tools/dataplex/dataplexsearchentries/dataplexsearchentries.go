...
func (cfg Config) Initialize(srcs map[string]sources.Source) (tools.Tool, error) {
	...
	scope := parameters.NewStringParameter("scope", "A scope limits the search space to a particular project or organization. It must be in the format: organizations/<org_id> or projects/<project_id> or projects/<project_number>.", parameters.WithStringDefault(""))
	pageSize := parameters.NewIntParameter("pageSize", "Number of results in the search page.", parameters.WithIntDefault(5))
	orderBy := parameters.NewStringParameter("orderBy", "Specifies the ordering of results. Supported values are: relevance, last_modified_timestamp, last_modified_timestamp asc", parameters.WithStringDefault("relevance"))
